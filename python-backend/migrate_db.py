...
def migrate_and_seed():
    ...
    with app.app_context():
        ...
        if "requirements" in tables:
            requirement_columns = [c["name"] for c in inspector.get_columns("requirements")]
            if "evaluation_period" not in requirement_columns:
                logger.info("Adding evaluation_period column to requirements table...")
                db.session.execute(text("ALTER TABLE requirements ADD COLUMN evaluation_period VARCHAR(255);"))
                db.session.commit()
                logger.info("Added requirements.evaluation_period.")
            else:
                logger.info("requirements.evaluation_period already exists.")
